...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Note: following are now also in org-sc package in tiny-sc repository:
  ...
  Optionally evaluate quoted lisp expression returned by SuperCollider."
    (setq sclang-return-string (ad-get-arg 1))
    (if (and (> (length sclang-return-string) 3)
             (equal (substring sclang-return-string 0 2) "'("))
        (eval (eval (read sclang-return-string)))))
