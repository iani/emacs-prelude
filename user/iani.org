...
#+BEGIN_SRC emacs-lisp
  ...
  (defun org-export-as-latex-with-header-from-file ()
    (interactive)
   (let* ((paths (file-expand-wildcards (concat latex-templates-path "/*.tex")))
          (names-and-paths
           (mapcar
            (lambda (x)
          ...
          latex-header
          (latex-sections
           (or (assoc this-buffers-latex-class latex-section-templates)
               (assoc "article" latex-section-templates))))
     (when chosen-path
       (setq latex-header
             (with-temp-buffer
       ...
       (setq org-latex-classes
             (list
              (append (list this-buffers-latex-class latex-header)
                      (cdr latex-sections)))))))
#+END_SRC
